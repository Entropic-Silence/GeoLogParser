...
def main() -> None:
    ...
    lines.extend([
        ...
        "- Paper II: `EXPERIMENTING`. Five mutually disjoint California evaluations show unselective F1 gains with FCR 0.084–0.210, while the frozen addition-only policy accepted 43/43 and 39/39 correct additions on v004/v005. On BGS v001 development, v018 reached interval F1 0.1116, v021 reached 0.1213, v022 reached 0.1475, and the nested source-disjoint v023 continuous-depth refiner reached boundary F1 0.3381 and interval F1 0.1797 at ±0.05 m. The v024 page-family/risk route reached boundary F1 0.3313 and interval F1 0.1801 on the same development panel, so it remains a reliability branch rather than a primary-method replacement. The new semantic column-role diagnostic reached boundary/interval F1 0.3265/0.1458 without fallback and 0.3094/0.1179 with no-anchor fallback; on its 9-document explicit-Graphic-Log subset it reached 0.4410/0.2825, but it did not pass the primary promotion gate. NativeMM direct and spatial heads remain `NO_GO` on BGS development (structural-evidence coverage 0.0–0.05 at the fixed operating points). The v023 model was frozen at commit 9ce6fd6 before the one-time 3-document/4-page/49-interval BGS v002 external run. External boundary precision/recall/F1 collapsed to 0.0227/0.0385/0.0286, interval F1 was 0, and selective precision was 0.0400 with CNER 0.9600. Corrected-page v002r2 validation of v024 achieved boundary precision/recall/F1 1.0000/0.0769/0.1429, interval F1 0.1154 and CNER 0.0000 by abstaining on the long-log and zero-evidence pages. v002 is consumed and no tuning is permitted. A replacement v003 freeze contains one unseen source title, five pages and seven intervals; it remains unopened and is too small to establish generalization alone. Pairwise ranking, post-sequence pruning, NativeMM and v024 primary-method promotion remain `NO_GO`.",

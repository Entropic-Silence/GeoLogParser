...
def main() -> None:
    ...
    lines = [
        ...
        f"| Published manual-transcription Gold | v001: {len(california_rows)} documents / {california_intervals} intervals with {len(california_test_rows)} documents / {california_test_intervals} intervals held out; v002 external: {len(california_external_rows)} documents / {california_external_intervals} intervals; v003 prospective: {len(california_prospective_rows)} documents / {california_prospective_intervals} intervals | USGS staff manually transcribed the paired WCR images and applied published depth-logic/completeness QC; project_human_reviewed=false |",
    ...
    lines.extend([
        ...
        "## Paper status",
        "",
        f"- Paper I: `RESULTS_AVAILABLE` for three mutually record-disjoint published-manual-transcription California evaluations: v001 {len(california_test_rows)} documents/{california_test_intervals} intervals, v002 {len(california_external_rows)} documents/{california_external_intervals} intervals, and prospective v003 {len(california_prospective_rows)} documents/{california_prospective_counties} counties/{california_prospective_pages} pages/{california_prospective_intervals} intervals. RapidOCR/Tesseract F1 was 0.390/0.325, 0.450/0.346, and 0.383/0.330. Additional real evidence includes a 26-source-group BGS historical-scan benchmark (341 official intervals; RapidOCR/Tesseract F1 0.0379/0.0405), a seven-document/608-page USGS Idaho source-scan disagreement audit, a 35-document/80-interval PDF-content-group held-out benchmark, a 46-record/3,332-interval source-disjoint official-database transfer panel, and a two-document/62-interval Raft River benchmark. Representative multilingual source-disjoint manually transcribed Gold remains `NOT COMPLETED`.",

...
def main() -> None:
    ...
    lines.extend([
        ...
        "## Paper status",
        "",
        f"- Paper I: `RESULTS_AVAILABLE` for four mutually record-disjoint published-manual-transcription California evaluations: v001 {len(california_test_rows)} documents/{california_test_intervals} intervals, v002 {len(california_external_rows)}/{california_external_intervals}, v003 {len(california_prospective_rows)}/{california_prospective_intervals}, and prospective v004 {len(california_v004_rows)} documents/{california_v004_counties} counties/{california_v004_pages} pages/{california_v004_intervals} intervals. RapidOCR F1 was 0.390, 0.450, 0.383, and 0.428; matched Tesseract comparisons exist for v001-v003. Additional real evidence includes a 26-source-group BGS historical-scan benchmark, a seven-document/608-page USGS Idaho source-scan disagreement audit, a 35-document/80-interval PDF-content-group held-out benchmark, a 46-record/3,332-interval source-disjoint official-database transfer panel, and a two-document/62-interval Raft River benchmark. Representative multilingual source-disjoint manually transcribed Gold remains `NOT COMPLETED`.",

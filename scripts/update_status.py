...
def main() -> None:
    ...
    lines = [
        ...
        f"| Published manual-transcription Gold | v001: {len(california_rows)} documents / {california_intervals} intervals with {len(california_test_rows)} documents / {california_test_intervals} intervals held out; v002 external: {len(california_external_rows)} documents / {california_external_intervals} intervals | USGS staff manually transcribed the paired WCR images and applied published depth-logic/completeness QC; project_human_reviewed=false |",

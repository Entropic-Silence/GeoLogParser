#!/usr/bin/env python3
"""Run a multi-seed synthetic protocol validation for Paper III."""
...
from geologparser.evaluation import (
    aggregate_repeated_metrics, boundary_surface_points, idw_predict,
    perturb_interval_boundaries, surface_error_metrics,
)
...
def main() -> None:
    ...
    if arguments.repetitions < 2:
        raise ValueError("multi-seed protocol requires at least two repetitions")
    commit = subprocess.run(
        ["git", "rev-parse", "HEAD"], cwd=ROOT, text=True, capture_output=True, check=True,
    ).stdout.strip()
    magnitudes = (0.01, 0.05, 0.10, 0.50, 1.00)
    run = create_run_directory(arguments.results_root, {
        "experiment_id": arguments.experiment_id,
        "git_commit": commit,
        "date": "2026-08-12",
        "dataset_version": "synthetic_four_borehole_protocol_fixture_v001",
        "split_version": "not_applicable_multiseed_protocol",
        "model": "idw_power_2",
        ...
        "config": {
            "magnitudes_m": list(magnitudes), "repetitions": arguments.repetitions,
            "interval_index": 0, "boundary": "bottom_depth_m", "idw_power": 2,
            "query_grid": {"x": [0, 2, 4, 6, 8, 10], "y": [0, 2, 4, 6, 8, 10]},
            "confidence_interval": "mean +/- 1.96 * sample_std / sqrt(n); protocol only",
            "scope": "synthetic multi-seed protocol validation; not real-world evidence",
        },
    })
    ...
    for magnitude_index, magnitude in enumerate(magnitudes):
        ...
        for repetition in range(arguments.repetitions):
            seed = arguments.base_seed + magnitude_index * 10000 + repetition
            perturbed = perturb_interval_boundaries(records, magnitude, seed)
            points = boundary_surface_points(perturbed, 0)
            surface = [idw_predict(points, x, y) for x, y in queries]
            measured = surface_error_metrics(reference_surface, surface)
            repetition_row = {"magnitude_m": magnitude, "seed": seed, **measured}
            repetitions.append(measured)
            rows.append(repetition_row)
    ...
    (run / "run.log").write_text(
        f"status=completed\nrepetitions={arguments.repetitions}\nscope=synthetic_multiseed_protocol\n",
        encoding="utf-8",
    )
    print(run)

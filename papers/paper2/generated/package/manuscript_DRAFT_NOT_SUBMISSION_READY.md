...
We study whether geological and numerical constraints can improve borehole-log extraction without silent rule-based overwriting. The method separates positioned OCR evidence, sequence candidates, depth monotonicity, boundary continuity, column stability, targeted rereading, ranking, confidence, and abstention. Its principal real evidence uses two non-overlapping California well-completion-report evaluations with 697 and 1,770 intervals manually transcribed and quality-controlled in a published USGS release. A deterministic sequence ranker frozen on ten v001 development reports increased interval F1 from 0.390 to 0.514 on the 50-report v001 test and from 0.450 to 0.564 on the 100-report external v002 set. False-correction rates were 18/109 (0.165) and 63/355 (0.177); 16/50 and 36/100 reports improved, while 5/50 and 8/100 worsened. Complementary evidence includes a 35-document Swissgeol held-out test where rereading raised F1 from 0.857 to 0.921 with FCR 0/4, a separate 20-document negative validation with no accepted correction, and a 31-document BGS consensus/abstention study. Constraints repeatedly improve recovery, but the stable harmful-correction rate shows that consistency is not correctness and that automatic acceptance remains unsafe without stronger evidence or abstention. <!-- evidence:p2.california_constraint_sequence --> <!-- evidence:p2.california_external_constraint_sequence --> <!-- evidence:p2.swissgeol_v2_heldout_constraint_reread -->
...
Categories include constraint false positive/negative, ambiguous ROI, OCR/VLM agreement on a wrong value, layout mislocalization, candidate omission, reread failure, normalization error, and unsafe correction. California exposes all three outcomes that an average F1 change can hide. On v001, wrong→correct comprised 81 added matches and 10 removed errors, correct→wrong comprised 12 added errors and six removed matches, and 448 reference intervals remained missed. On v002 these counts were 212, 80, 17, 46, and 1,052 respectively; eight reports worsened despite globally improved precision and recall. The v1 Swissgeol result exposed a different blind spot: all three incorrect first-pass documents produced plausible sequences that evaded every trigger. On v003, v2 recovered four complete documents without a false correction, but three of ten erroneous documents still evaded triggering and three triggered errors remained unresolved. In the Padova audit, a provenance box spanning several numeric fields caused the constraint score to favor a semantically wrong depth candidate; abstention prevented acceptance. Together these cases show that topology narrows candidates but cannot replace field localization and source evidence. <!-- evidence:p2.california_external_constraint_sequence -->

## 7. Discussion

Constraint consistency is not correctness: a plausible but wrong continuous sequence can pass. Two disjoint California evaluations quantify this risk with FCR 0.165 over 109 actions and 0.177 over 355 actions, while the Swissgeol studies show both trigger blindness and a small positive correction set. Weak constraints must not dominate pixel evidence. The California gain is attributable to the complete sequence-selection policy as a package—positioned OCR hypotheses, semantic filtering, monotonicity, continuity, and layout-column stability—not to any single geological constraint. The v002 replication strengthens external validity but does not provide component attribution or a prospectively frozen abstention threshold. The next confirmatory design must separate candidate generation and ranking, fit abstention on development only, and measure coverage versus residual error on a new frozen test.
...
We provide an implemented constraint-guided extraction architecture and evaluate both recovery and harm. On two non-overlapping California sets totaling 150 reports and 2,467 published manually transcribed intervals, frozen sequence ranking raised F1 from 0.390 to 0.514 and from 0.450 to 0.564. FCR remained non-negligible at 18/109 and 63/355, and 13 reports worsened across the two sets. A separate Swissgeol test improved from F1 0.857 to 0.921 with FCR 0/4, whereas another frozen application produced no correction. The evidence supports geology-aware sequence reasoning as a recovery mechanism, but not unconditional automatic repair. Reliable deployment requires development-fitted abstention, explicit correction-risk reporting, and stronger field-localized evidence. <!-- evidence:p2.california_external_constraint_sequence -->

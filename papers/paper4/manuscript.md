...
The GeoLogParser repository contains versioned code/configuration, prompt hashes, metric bindings, and scripts that regenerate the tables, figures, claim audit, submission gate, and reproducibility package for this article. The source code is released under the MIT license at https://github.com/Entropic-Silence/GeoLogParser. The complete article and result-reproduction package is identified by the annotated release tag `paper4-cageo-v1.0.1`; the archival DOI will be added after deposit.

...
The current authoritative interval estimate uses the v003 PDF-content-group held-out set with 35 documents and 80 numeric intervals, disjoint by record and PDF hash from the 37-document development partition. The frozen raster path predicted 74 intervals without reading official final depth or any other reference field. Sixty-six predictions matched within ±0.05 m, yielding micro precision 0.892, recall 0.825, and F1 0.857. Twenty-five of 35 documents were completely exact. Fourteen reference intervals were unmatched and eight predictions were spurious. Matched top- and bottom-boundary MAE were both 0.000 m. Mean wall time was 3.047 s/document on CPU. Earlier runs remain separated by evidential role: run 001 is oracle-conditioned, run 002 is development evidence, run 003 is the first 20-document incremental held-out estimate, and run 004 is the larger content-group held-out estimate reported here. The reference remains source-agreement-selected rather than representative of cross-source deployment. <!-- evidence:p1.swissgeol_authoritative_interval -->
...
We present the executable definition and infrastructure for a provenance-aware heterogeneous borehole-log benchmark, report a real authoritative-metadata comparison that reveals severe field-dependent reliability, and establish a hash-bound PDF-content-group held-out interval test of 35 documents and 80 source-agreement intervals. The frozen reference-independent baseline achieved precision 0.892, recall 0.825, F1 0.857, and complete-document exactness of 25/35. Its zero matched-boundary MAE nevertheless coexisted with 14 missing and eight spurious intervals, demonstrating why structural errors and coverage must accompany conditional boundary error. A representative conclusion about multilingual interval/lithology performance, source-disjoint generalization, or random-split inflation remains `TBD` until the broader experiments are complete.

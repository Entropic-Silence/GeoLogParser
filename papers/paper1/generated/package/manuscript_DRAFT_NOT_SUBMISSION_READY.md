...
Legacy borehole logs encode coordinates, elevations, interval boundaries, and geological descriptions in heterogeneous tables, scans, and drawings. Existing OCR scores do not establish whether these records can be converted into reliable databases, and page-level random splits can leak project templates. We define a provenance-bearing extraction task, a panel-aware annotation unit, leakage-resistant split protocols, a four-level evaluation framework, degradation metadata, and an error taxonomy. A 31-document/106-page British Geological Survey authoritative-metadata sub-benchmark provides the first real-source comparison at matched 300-DPI rendering: Tesseract and RapidOCR reached borehole-ID exact matches of 25/31 and 31/31 respectively, but neither recovered final depth reliably. A controlled first-page robustness extension evaluates 217 derived images (31 scans × 7 profiles) for ID/X/Y only: Tesseract's complete three-field exact rate fell from 24/31 on clean pages to 9/31 under 3° skew, while RapidOCR remained at 31/31 under the same skew but fell to 7/31 under JPEG quality 30. These robustness references are authoritative metadata, not interval/lithology labels. The rights-cleared interval/lithology benchmark size, split-generalization gap, and human agreement remain `TBD`. The intended contribution is a reproducible benchmark centered on data quality, provenance, critical-field coverage, and cross-template generalization rather than a new network.
...
B1 OCR+regex; B2 OCR+LLM; B3 OCR+layout+rules; B4 zero-shot VLM; B5 few-shot VLM; B6 OCR+VLM fusion. Adapters and model revisions come from a registry. Current executed comparisons cover Tesseract+regex and RapidOCR+regex on 31 BGS documents, B2 text-only and B3 positioned-text paths on Padova, plus Apache-2.0 Qwen3-VL-4B at fixed revision `ebb281ec...` through local B4/B5 adapters. B2 supplies only flattened PDF/OCR text to the language stack; B3 requires at least three distinct depth ranges in one repeated x-position bin and abstains when boundaries exist only as graphics. B4 uses greedy decoding and versioned prompt `vlm_extract_v002`; whole-image values are marked `VLM_UNGROUNDED`. B6 conservatively retains grounded evidence on disagreement and sends visual-only or conflicting fields to review. Formal interval/lithology runs are `TBD`.
...
We present the executable definition and infrastructure for a provenance-aware heterogeneous borehole-log benchmark and report a real authoritative-metadata comparison that reveals severe field-dependent reliability. A publishable conclusion about interval/lithology performance or split leakage is `TBD` until the corresponding reference set and experiments are complete.

...
B1 OCR+regex; B2 OCR+LLM; B3 OCR+layout+rules; B4 zero-shot VLM; B5 few-shot VLM; B6 OCR+VLM fusion. Adapters and model revisions come from a registry. Current executed comparisons cover Tesseract+regex and RapidOCR+regex on 31 BGS documents, a held-out Tesseract interval-boundary test on 20 Swissgeol records, B2 text-only and B3 positioned-text paths on Padova, plus Apache-2.0 Qwen3-VL-4B at fixed revision `ebb281ec...` through local B4/B5 adapters. The Swissgeol B1 run renders every PDF page at 250 DPI, applies Tesseract `eng` with PSM 3, and conservatively parses explicit interval tables; native PDF text is not used for prediction. Its parsing policy was frozen on the v001 development records before the disjoint incremental v002 test was evaluated. B2 supplies only flattened PDF/OCR text to the language stack; B3 requires at least three distinct depth ranges in one repeated x-position bin and abstains when boundaries exist only as graphics. B4 uses greedy decoding and versioned prompt `vlm_extract_v002`; whole-image values are marked `VLM_UNGROUNDED`. B6 conservatively retains grounded evidence on disagreement and sends visual-only or conflicting fields to review. Formal multi-model interval and lithology comparisons remain `TBD`.
...
The authoritative interval estimate uses the incremental 20-document/24-page Swissgeol set with 55 numeric intervals, disjoint from all nine v001 parser-development records. The frozen raster path predicted 55 intervals without reading official final depth or any other reference field. Forty-seven predictions matched a reference interval within ±0.05 m, yielding micro precision, recall, and F1 of 0.855. Seventeen of 20 documents were completely exact. Across the other three documents, eight reference intervals were unmatched and eight predictions were spurious. Matched top- and bottom-boundary MAE were both 0.000 m. Mean wall time was 3.832 s/document on CPU. Three runs are retained with distinct evidential roles: run 001 supplied official final depth during filtering/ranking and is an oracle-conditioned diagnostic; run 002 was reference-independent but used the nine development records and is excluded from held-out claims; run 003 is the disjoint incremental held-out estimate reported here. The reference set remains source-agreement-selected rather than a representative cross-source sample. <!-- evidence:p1.swissgeol_authoritative_interval -->
...
We present the executable definition and infrastructure for a provenance-aware heterogeneous borehole-log benchmark, report a real authoritative-metadata comparison that reveals severe field-dependent reliability, and establish a hash-bound incremental held-out interval test of 20 documents and 55 source-agreement intervals. The frozen reference-independent baseline achieved precision = recall = F1 = 0.855 and complete-document exactness of 17/20. Its zero matched-boundary MAE nevertheless coexisted with eight missing and eight spurious intervals, demonstrating why structural errors and coverage must accompany conditional boundary error. A representative conclusion about heterogeneous interval/lithology performance or split leakage remains `TBD` until the broader reference set and experiments are complete.

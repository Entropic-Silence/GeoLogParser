...
Legacy borehole logs encode numerical boundaries and geological descriptions in heterogeneous tables, scans, and drawings, yet ordinary OCR scores do not establish database reliability. We define a provenance-bearing structured-extraction task, leakage-resistant splits, four-level evaluation, degradation metadata, and a critical-error taxonomy. The principal Gold evidence pairs public redacted California Department of Water Resources reports with a USGS release manually transcribed from the images and quality-controlled. A 50-report/697-interval held-out test and a non-overlapping 100-report/1,770-interval external replication were frozen. RapidOCR obtained interval F1 0.390 and 0.450 on the two sets, versus 0.325 and 0.346 for Tesseract; the external set contained 1,220 and 1,378 missed reference intervals despite precision 0.817 and 0.789. RapidOCR recovered more external-set matches on 46 reports, Tesseract on 20, and they tied on 34. Complementary experiments include a 35-document/80-interval Swissgeol source-agreement test, a two-document/62-interval Raft River table benchmark, a 31-document BGS metadata benchmark with 217 controlled degraded images, and a five-canton source-disjoint transfer panel. Performance ranges from perfect boundary recovery on a tailored Raft River layout to near-total transfer collapse on 3,332 external database intervals. These results show that high conditional precision and zero matched-boundary MAE can coexist with severe omissions, lithology errors, and source dependence. Broader multilingual manually transcribed Gold, random-versus-grouped split inflation, and independent human agreement remain `TBD`. <!-- evidence:p1.california_rapidocr_gold --> <!-- evidence:p1.california_external_rapidocr --> <!-- evidence:p1.california_external_tesseract --> <!-- evidence:p1.swissgeol_authoritative_interval -->
...
We present a provenance-aware heterogeneous borehole-log benchmark anchored by two non-overlapping California evaluations totaling 150 reports and 2,467 published manually transcribed test intervals. RapidOCR/Tesseract F1 was 0.390/0.325 on v001 and 0.450/0.346 on external v002. Both evaluations show backend-specific failures, severe omission, and incomplete lithology recovery despite high conditional precision. Complementary Swissgeol, Raft River, BGS, and five-canton experiments span source-specific success, degradation sensitivity, and complete transfer collapse. The consistent finding is that conditional numerical accuracy alone materially overstates deployable reliability: omission, column semantics, and source shift dominate. Broader multilingual manually transcribed Gold and a direct random-versus-grouped split experiment remain `TBD`. <!-- evidence:p1.california_external_rapidocr --> <!-- evidence:p1.california_external_tesseract -->

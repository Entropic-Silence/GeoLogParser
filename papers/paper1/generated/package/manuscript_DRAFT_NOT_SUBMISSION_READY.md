...
The BGS experiments demonstrate why coverage must accompany MAE: zero extracted final-depth values yield undefined MAE, not zero error. The matched 31-document comparison also exposes complementary critical failures: Tesseract made six ID errors while abstaining on final depth, whereas RapidOCR's sole depth prediction was a 133.48 m error. The controlled robustness extension shows that degradation sensitivity is backend-specific even when the reference fields are simple metadata: 3° skew was severe for Tesseract but not RapidOCR, while JPEG quality 30 was severe for RapidOCR. The Swissgeol held-out comparison shows a second metric hazard: zero MAE on matched boundaries coexisted with substantial omission and spurious-interval differences between engines. The Raft River comparison adds a field-semantic hazard: a layout-aware backend recovered every boundary yet still assigned a water-column mark as lithology once, while line OCR suffered both separator loss and large digit substitution. The five-canton transfer result supplies the stronger generalization warning: a parser that reached F1 0.857 on source-agreement-selected Thurgau records produced candidates on only 2/46 external records and reached at most F1 0.0006 against their official database sequences. Changing OCR backend did not restore coverage, and long-format Aargau records exposed both extreme runtime scaling and total sequence omission. Structural matching, content-group weighting, complete-document exactness, backend identity, unmatched counts, page geometry, field semantics, and explicit source/reference tiers are therefore necessary alongside conditional boundary error. Because the external page/database sequences were not fully verified, the result establishes failure of transfer agreement, not a pure extraction-error rate or representative cross-canton Gold accuracy.
...
We present the executable definition and infrastructure for a provenance-aware heterogeneous borehole-log benchmark, report real authoritative-metadata comparisons across two OCR backends, and establish a hash-bound PDF-content-group held-out interval test of 35 documents and 80 source-agreement intervals. Tesseract achieved F1 0.857 and RapidOCR F1 0.679 on that selected reference-independent split; both had zero matched-boundary MAE, but their unmatched-interval profiles differed substantially. A separate two-document Raft River table benchmark added 62 source-explicit intervals: RapidOCR recovered every boundary and 61/62 lithology strings, whereas Tesseract reached F1 0.831 and only 4/49 exact lithology strings among boundary matches. On 46 external paired records from five non-development cantons, however, frozen-parser transfer agreement fell to F1 0.0006 and 0.0000, with interval candidates on only two records. The four incremental Aargau long logs added 2,545 official intervals yet produced no candidate under either backend. The cross-source evidence therefore spans near-perfect layout-specific extraction, severe OCR/column-semantic errors, and complete transfer collapse. The external database reference cannot isolate extraction error from page/source mismatch; nevertheless, it directly falsifies any claim that the Thurgau result alone represents cross-source deployment. Representative multilingual interval/lithology performance, broader source-disjoint interval Gold, and random-split inflation remain `TBD`.

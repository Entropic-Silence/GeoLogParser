...
We present the executable definition and infrastructure for a provenance-aware heterogeneous borehole-log benchmark, report real authoritative-metadata comparisons across two OCR backends, and establish a hash-bound PDF-content-group held-out interval test of 35 documents and 80 source-agreement intervals. Tesseract achieved F1 0.857 and RapidOCR F1 0.679 on the same reference-independent split; both had zero matched-boundary MAE, but their unmatched-interval profiles differed substantially. These results show why structural coverage and backend identity must accompany conditional numeric error. A representative conclusion about multilingual interval/lithology performance, source-disjoint interval Gold, or random-split inflation remains `TBD` until the broader experiments are complete.

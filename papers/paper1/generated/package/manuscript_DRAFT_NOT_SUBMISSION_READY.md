...
Legacy borehole logs encode numerical boundaries and geological descriptions in heterogeneous tables, scans, and drawings, yet ordinary OCR scores do not establish database reliability. We define a provenance-bearing structured-extraction task, leakage-resistant splits, four-level evaluation, degradation metadata, and a critical-error taxonomy. The primary Gold test pairs public redacted California Department of Water Resources reports with a USGS release whose 697 held-out intervals were manually transcribed from the images and quality-controlled. The fixed test contains 50 reports from 48 counties and 77 pages. RapidOCR emitted 195 intervals, matched 174 (precision 0.892, recall 0.250, F1 0.390), and exactly recovered lithology for 75/174 boundary matches; Tesseract emitted 176, matched 142 (precision 0.807, recall 0.204, F1 0.325), and recovered 30/142 lithology strings. Eleven and twelve reports respectively produced no interval prediction. Complementary experiments include a 35-document/80-interval Swissgeol source-agreement test, a two-document/62-interval Raft River table benchmark, a 31-document BGS metadata benchmark with 217 controlled degraded images, and a five-canton source-disjoint transfer panel. Performance ranges from perfect boundary recovery on a tailored Raft River layout to near-total transfer collapse on 3,332 external database intervals. These results show that high conditional precision and zero matched-boundary MAE can coexist with severe omissions, lithology errors, and source dependence. Broader multilingual manually transcribed Gold, random-versus-grouped split inflation, and human agreement remain `TBD`. <!-- evidence:p1.california_rapidocr_gold --> <!-- evidence:p1.california_tesseract_gold --> <!-- evidence:p1.swissgeol_authoritative_interval -->
...
Post-hoc document-level analysis showed that RapidOCR recovered more matched intervals on 22 reports, Tesseract on 15, and the engines tied on 13. RapidOCR had 11 zero-output reports, 21 spurious intervals, and 99 lithology errors among its 174 boundary matches; Tesseract had 12 zero-output reports, 34 spurious intervals, and 112 lithology errors among 142 matches. The dominant real failure was therefore whole-row or whole-report omission, followed by column contamination and semantic-string corruption rather than small numerical deviation among accepted matches. Development-set engine ordering did not remove this source dependence: Tesseract PSM 11 had the higher development F1 (0.477 versus RapidOCR's selected 0.425), but RapidOCR was better on the blind test (0.390 versus 0.325). <!-- evidence:p1.california_error_analysis -->
...
The California result supplies the clearest warning: precision above 0.80 and zero conditional boundary MAE coexist with recall below 0.25, 11–12 complete report failures, and low lithology exactness. The BGS experiments similarly show why coverage must accompany MAE: zero extracted final-depth values yield undefined MAE, not zero error. Controlled degradation sensitivity is backend-specific: 3° skew was severe for Tesseract but not RapidOCR, while JPEG quality 30 was severe for RapidOCR. The Swissgeol result adds a source-selection hazard, and Raft River adds a field-semantic hazard: a layout-aware backend recovered every boundary yet assigned a water-column mark as lithology once. The five-canton transfer result is stronger still: a parser with F1 0.857 on source-agreement-selected Thurgau records produced candidates on only 2/46 external records. Structural matching, complete-document exactness, unmatched counts, page geometry, field semantics, backend identity, and explicit reference tiers are therefore necessary alongside conditional error. The California sample is county-diverse but was filtered for continuity, empty comments, and moderate interval count; it is not a random sample of every California report type.
...
We present a provenance-aware heterogeneous borehole-log benchmark anchored by 50 held-out California reports and 697 published manually transcribed intervals. RapidOCR reached precision 0.892 but recall 0.250 (F1 0.390), while Tesseract reached precision 0.807 and recall 0.204 (F1 0.325). The two backends failed on different reports and exactly recovered only 43.1% and 21.1% of lithology strings among boundary matches. Complementary Swissgeol, Raft River, BGS, and five-canton experiments span source-specific success, degradation sensitivity, and complete transfer collapse. The consistent finding is that conditional numerical accuracy alone materially overstates deployable reliability: omission, column semantics, and source shift dominate. Broader multilingual manually transcribed Gold and a direct random-versus-grouped split experiment remain `TBD`.

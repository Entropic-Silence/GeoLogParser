...
Legacy borehole logs combine numerical boundaries, geological descriptions, and source-specific column conventions; ordinary OCR scores therefore do not establish database reliability. We define a provenance-bearing structured-extraction task and separate five evidence tiers before scoring. The primary accuracy evidence comprises five mutually record-disjoint California cohorts containing 450 reports and 8,268 intervals manually transcribed and quality-controlled by the publishing institution. A fixed RapidOCR parser obtained interval F1 values of 0.390, 0.450, 0.383, 0.428, and 0.389; document-cluster 95% intervals were [0.292, 0.489], [0.370, 0.526], [0.307, 0.460], [0.341, 0.513], and [0.308, 0.472]. Across cohorts, zero-output rates were 8%–23%, and only 1%–6% of reports had an exact boundary sequence. Source-shift diagnostics were deliberately reported at their weaker evidence tiers: a 35-document Swissgeol source-agreement panel reached F1 0.857 with Tesseract, whereas a 26-document BGS historical-scan panel yielded F1 0.0379/0.0405 for RapidOCR/Tesseract, and a five-canton transfer panel collapsed to micro F1 at or below 0.0006. Conditional precision and matched-boundary MAE repeatedly concealed whole-report omissions, column-semantic errors, and source dependence. The study is consequently a multi-cohort and cross-source evaluation, not a claim of a comprehensive multilingual benchmark. <!-- evidence:p1.california_rapidocr_gold --> <!-- evidence:p1.california_external_rapidocr --> <!-- evidence:p1.california_prospective_rapidocr --> <!-- evidence:p1.california_v004_rapidocr --> <!-- evidence:p1.california_v005_rapidocr --> <!-- evidence:p1.california_replication_statistics --> <!-- evidence:p1.swissgeol_authoritative_interval --> <!-- evidence:p1.bgs_offshore_rapidocr --> <!-- evidence:p1.bgs_offshore_tesseract -->

## 1. Introduction

Borehole logs are machine-readable only when numerical boundaries, geological text, layout, and source evidence remain linked. Treating a page as plain OCR text loses column semantics; treating it as an unconstrained image-to-JSON prompt obscures failures and provenance. This paper asks: (RQ1) how stable is a frozen parser across independent batches from one manually transcribed source; (RQ2) how far does performance transport across page families and institutions; (RQ3) when do conditional precision and matched-boundary error conceal non-output and structural failure; and (RQ4) which error mechanisms dominate the observed source shift?

The paper owns the task definition, evidence hierarchy, representative reproducible baselines, multi-cohort stability analysis, and source-shift findings. Sequence reconstruction and correction-risk policies belong to Paper II; downstream spatial propagation belongs to Paper III. Related manuscripts reuse only the minimum raw baseline needed for comparison and disclose the relationship to editors.

## 2. Related Work

OCR engines such as Tesseract provide a stable recognition layer but do not recover domain-specific table semantics [@smith2007tesseract]. LayoutLM and LayoutLMv2 jointly model text, spatial position, and later image features [@xu2020layoutlm; @xu2021layoutlmv2]. PubLayNet and DocLayNet provide general layout labels [@zhong2019publaynet; @pfitzmann2022doclaynet], while PubTables-1M evaluates table structure with topology-aware measures [@smock2022pubtables]. OCR-free document models such as Donut and high-resolution document parsers such as DocOwl2 show that text recognition and structure reasoning can be learned jointly [@kim2022donut; @hu2024docowl2]. These advances motivate representative OCR, positioned-text, and local-VLM baselines here, but none supplies borehole interval identity, depth topology, or field provenance by itself.

Han and Suh provide the closest verified borehole-specific comparison: 908 pages from 47 Korean abandoned-mine reports were grouped into five page types, with classification and Type-1 spreadsheet structuring evaluated separately [@han2024boreholeocr]. The present study differs by making interval boundaries, missing output, document exactness, source evidence, and cross-source transport the evaluation target. It does not claim a broader multilingual Gold corpus than that prior work.

Dependence-aware validation literature cautions that observations sharing spatial, temporal, or hierarchical structure are not exchangeable [@roberts2017crossvalidation]. Borehole reports similarly share templates, producers, and acquisition conditions. Our five California cohorts are therefore record-disjoint, and source-shift panels are reported separately rather than pooled as equivalent Gold. A frozen-prediction random/grouped resampling is retained only as a supplementary diagnostic: because it retrains nothing, it cannot estimate general random-split optimism.

| Closest work | Primary target | Difference in this study |
|---|---|---|
| General layout and table benchmarks [@zhong2019publaynet; @pfitzmann2022doclaynet; @smock2022pubtables] | Regions and table topology | Ordered geological intervals, numerical boundaries, non-output, and provenance |
| OCR-free/high-resolution document models [@kim2022donut; @hu2024docowl2] | Generic document transcription or question answering | Frozen, evidence-tiered source transport with document-level failure accounting |
| Han and Suh [@han2024boreholeocr] | Page-type classification and one-type structuring | Multi-cohort interval metrics and cross-source stress tests; no page-type score comparison |
| Dependence-aware validation [@roberts2017crossvalidation] | Evaluation under grouped dependence | Cohort- and source-separated reporting, without claiming a retraining-based leakage effect |
...
A separate USGS Idaho source-scan audit broadens the real image-only evidence without converting machine agreement into Ground Truth. Seven official lithologic-log PDFs (USGS 144, 145, 150, 152/152A/152B, and CFPP-B01) contain 608 pages. At 180 DPI, Tesseract emitted non-empty text on 608 pages and RapidOCR on 607. Both detected an explicit `LITHOLOGY:` label on 281 pages; RapidOCR alone detected the label on 49 pages and Tesseract alone on none. The 49 disagreements were concentrated in USGS 144 (2 pages), USGS 145 (33), and USGS 150 (14), whereas all three USGS 152 logs and CFPP-B01 had zero label-presence disagreement. RapidOCR detected 3,107 numeric depth ranges versus 2,922 for Tesseract, a difference of 185 candidates. These are reproducible cross-engine detection events, not accuracy measurements: neither engine is treated as the reference and no interval precision, recall, or F1 is reported. <!-- evidence:p1.usgs_idaho_cross_engine --> <!-- evidence:p1.usgs_idaho_error_analysis -->
...
Independent agreement uses two service-separated, full-overlap annotation tracks created from byte-identical frozen `auto` proposals before review. Each service fixes its actor ID server-side, so a browser payload cannot select the peer actor; this binds a process to a track but does not authenticate the human user. Shared-host filesystem and human access therefore remain deployment/study controls. Agreement is computed only after both tracks independently pass the human GT gate, rejects overlapping annotator IDs, and freezes aggregate metrics, every v001 field disagreement, record hashes, and source annotation hashes before adjudication. The adjudication builder writes both answers and pending discrepancy cases but never creates final GT automatically; even equal answers require source confirmation. The current Padova assignment contains 15 source pages and two tracks (30 task files), but all task files remain `auto`: there are zero effective human attestations, zero GT-exportable track annotations, zero agreement artifacts, and zero adjudication manifests. <!-- evidence:p1.padova_annotation_assignment --> These are workflow-readiness counts, not annotation sample size or agreement results. Inter-annotator, repeated self-agreement, and adjudication results are outside the evidence reported in this paper.

### 4.3 Metadata and splits

Metadata covers source, project, template, native/scanned/image type, resolution, blur/noise/skew, stamps, handwriting, and artifacts. Group IDs are assigned before any split. The v001 primary test is county-first and disjoint from its ten-report development partition. A separate 100-seed random-record calculation reuses frozen predictions and is retained only as a resampling diagnostic; it is not described as a training-split leakage experiment.

### 4.4 Degradation benchmark

Real and synthetic degradation cover resolution, blur, noise, skew, JPEG compression, contrast reduction, broken lines, watermarks, stamps, and occlusion. Every synthetic parameter and source hash is saved. A protocol-only Padova input set contains 270 deterministic derivatives (15 pages × 18 profiles), with manifest SHA256 `ca6bc6d6f2eff3df6916b3a87d43f24df6dacb13f4e924048b79120a339c5ba9`; it has no accuracy values because the source pages are not human-annotated. <!-- evidence:p1.degradation_inventory --> The BGS robustness set contains 217 first-page derivatives from 31 real scans, 300-DPI source rendering, seven profiles (`clean`, resolution 0.50, blur radius 2.0, Gaussian noise 16, 3° skew, JPEG quality 30, contrast 0.40), and manifest SHA256 `5a1205c944a5cb652967b99e6478169805db35f749dffe44124076eafd766628`. It evaluates only official borehole ID/X/Y references; final depth, intervals, and lithology are excluded. <!-- evidence:p1.bgs_robustness_inventory --> Training variants never enter test. No Chinese benchmark severity grid is included because the rights-cleared Chinese benchmark contains no eligible samples.

## 5. Baselines

The study uses representative reproducible baselines spanning OCR, positioned-text parsing, and a local VLM; it is not a state-of-the-art leaderboard. Tesseract 4.1.1 is retained as a stable, widely deployable reference engine, while RapidOCR supplies a second positioned-text backend. B2–B6 denote OCR+LLM, OCR+layout, zero-shot VLM, few-shot VLM, and OCR+VLM fusion adapters where data permit. Model revisions and prompts are versioned. California reports are rendered at 300 DPI; three RapidOCR development variants and one Tesseract configuration were compared only on the ten-report development set, then frozen. Swissgeol runs render every page at 250 DPI and conservatively parse explicit interval tables. Whole-image VLM values without a localized source region are marked `VLM_UNGROUNDED`. These baselines test distinct failure surfaces rather than claiming comprehensive model coverage.

## 6. Evaluation

Level 1 reports CER, explicitly segmented WER, and numeric CER where references exist. Level 2 reports exact categorical matches, normalized description similarity, and numeric coverage plus paired MAE. Level 3 uses order-preserving, boundary-aware interval precision/recall/F1 and boundary MAE at declared tolerances. Level 4 reports component geological consistency with explicit evaluated coverage. Documents are the primary uncertainty unit: interval totals are pooled descriptive counts, while confidence intervals are obtained by resampling whole reports. Every main table identifies its evidence tier, zero-output document rate, and complete-document exact rate. Latency, RAM/VRAM, tokens, and cost are logged.

The review-focused statistical table is generated directly from the frozen analysis JSON in [major-revision tables](generated/major_revision_tables.md); its California rows contain document-cluster intervals, zero-output rates, and exact-record rates. The broader indexed result catalogue remains in [current results](generated/current_results.md).
...
The formal B4 page aggregation used the same 50-report/77-page Gold test without reference conditioning. It produced schema-valid records on all 77 pages but only five interval candidates in one report; none matched a published boundary within ±0.05 m, giving interval precision, recall, and F1 of 0.000 and zero boundary-exact reports. The result is a measured end-to-end VLM baseline, not a parse-rate proxy: page-level JSON validity did not translate into interval recovery. The run retained 68.780 s/report of summed page-generation latency (the four GPU shards were executed independently), while the aggregation wall time was 0.0004 s/report. <!-- evidence:p1.california_vlm_gold -->
...
As a supplementary diagnostic, a 100-seed resampling pooled the ten v001 development records with the 50 county-first test records while freezing every prediction. Random-record F1 averaged 0.394 ± 0.021 versus 0.390 on the grouped test. This analysis neither retrains nor retunes the parser and therefore found no measurable F1 inflation for this frozen prediction set; it does not test the general causal claim that random training splits inflate performance. <!-- evidence:p1.random_vs_grouped_split -->
...
California cohort stability and uncertainty are shown in the [document-cluster forest plot](generated/figures/california_cohort_forest.png). Paired backend and sequence comparisons remain in [supplementary bootstrap intervals](generated/figures/california_replication.png).
Additional audit visuals are supplementary: [audit coverage](generated/figures/audit_coverage.png)
...
We present a provenance-aware multi-cohort and cross-source evaluation anchored by five mutually record-disjoint California cohorts totaling 450 test reports, 673 pages, and 8,268 published manually transcribed intervals. RapidOCR F1 was 0.390, 0.450, 0.383, 0.428, and 0.389 on v001–v005; document-cluster uncertainty, 8%–23% zero-output rates, and 1%–6% boundary-exact rates show that these pooled scores do not imply record completeness. Complementary Swissgeol, Raft River, BGS, USGS Idaho, and five-canton analyses are retained as separately tiered source-shift evidence rather than pooled into a multilingual Gold benchmark. The fixed-prediction random-record diagnostic observed no large F1 inflation, but it neither retrained nor retuned the parser and therefore supports no general leakage claim. The consistent result is narrower and more useful: conditional numerical accuracy alone materially overstates deployable reliability because omission, column semantics, and source dependence dominate. <!-- evidence:p1.california_replication_statistics --> <!-- evidence:p1.california_prospective_rapidocr --> <!-- evidence:p1.california_prospective_tesseract --> <!-- evidence:p1.usgs_idaho_cross_engine --> <!-- evidence:p1.random_vs_grouped_split -->
...
Acquisition inventories, no-GT coverage audits, Silver-agreement experiments, CAD conversion diagnostics, and source-specific software checks are organized in [Supplementary Material](supplement.md). They are excluded from the four main result groups because their evidence tiers do not support formal extraction-accuracy claims.
...
This supplement retains acquisition, eligibility, degradation-inventory, CAD-conversion, Machine-Silver, no-reference coverage, and single-source diagnostic material that is useful for reproducibility but cannot support the same claims as Published manual transcription Gold. The main manuscript limits performance inference to explicitly tiered California, Swissgeol, BGS, Raft River, and source-shift results.

## S2. Candidate-source and annotation-system audits

- Padova, slopes/Tiber, SedLog, and Chinese CAD candidates retain licence, retrieval, renderer, source-review, and annotation-state manifests.
- Padova duplicate-track infrastructure contains no effective human attestation and supplies no project-created Gold result.
- Chinese CAD conversion and four-panel VLM runs remain rights-unverified or conversion-incomplete and are excluded from accuracy tables.

## S3. Secondary diagnostics

- USGS-142 and USGS-144 are tailored single-document transfer checks.
- USGS Idaho cross-engine counts are detection-agreement events, not accuracy.
- BGS degradation transforms evaluate identifier/coordinate fields only.
- Padova Machine-Silver comparisons report agreement to a machine-constructed reference.
- Slopes, Tiber, SedLog, Padova, and quarantined Chinese runs report coverage, schema validity, candidate counts, latency, or failure type only.

All exact experiment IDs, evidence tiers, hashes, and numeric outputs remain in [current results](generated/current_results.md), the claim registry, and the publication-evidence bundle.

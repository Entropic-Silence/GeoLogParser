...
def explicit_interval_sections(text: str, final_depth_m: float) -> list[list[tuple[float, float]]]:
    ...
    for index, line in enumerate(lines):
        ...
        if (
            ...
            or (re.search(r"\btiefe\b", lowered) and re.search(r"beschreibung.*bohrgut", lowered))
        ):
            starts.append(index)
    ...
    for start in starts:
        ...
        for raw_line in lines[start + 1:start + 56]:
            ...
            if match:
                ...
                if 0 <= top < bottom <= final_depth_m + 1e-6:
        ...
        if ranges:
            ...
            for boundary in boundaries:
                ...
                if abs(boundary - final_depth_m) <= 1e-6:
                    break
            increasing = []
...
def choose_interval_section(text: str, final_depth_m: float) -> list[tuple[float, float]]:
    sections = explicit_interval_sections(text, final_depth_m)
    return max(
        sections,
        key=lambda section: (
            abs(section[-1][1] - final_depth_m) <= 1e-6,
            len(section),
        ),
        default=[],
    )
